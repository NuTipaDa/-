...
/*
Если класс Circle является подклассом класса Point, то у класса-потомка Circle есть все те же
свойства, что и у класса-предка Point, плюс, возмоэжно, какие-то ещё

Код не скомпилируется, потому что x и y объявлены в базовом классе приватными, а значит в
методах других классов, даже собственных потомков, эти свойства недоступны:
Но не делать же их публичными?! Именно для такой ситуации и существует своеобразный промежуточный вариант с использованием свойства protected: обозначенные этим
модификатором свойства и методы доступны не только в методах самого класса, но и в
методах классов-потомков:
*/
class Point {
protected:
	int x;
